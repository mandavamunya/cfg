...
public class CFG {
	...
	/**
	 * Check if the language generated by this CFG is finite. TODO: IMPLEMENT
	 */
	public boolean languageIsFinite() {
		CFG g = this.removeEpsilonRules();
		g = g.removeUnitRules();
		g = g.removeUselessSymbols();
		return !g.isCyclicInit(startSymbolID);
	}
	...
	public CFG removeUnitRules() {
		...
		createNonUnitRules();
		hasVisitedSymbol = new boolean[nts.size()];
		findUnitRules(startSymbolID);
		for (int ntID = 0; ntID < nonUnitRules.size(); ntID++) {
			String nt = symbolIDToString(ntID);
			for (int bodyID : nonUnitRules.get(ntID)) {
				ArrayList<Integer> body = bodies.get(bodyID);
				StringBuilder rule = new StringBuilder(nt + " -> ");
				for (int symbolID : body) {
					rule.append(symbolIDToString(symbolID));
	...
	/**
	 * Create an list of sets that is identical to the variable "rules", except
	 * that it will not contain any unit rule.
	 */
	private void createNonUnitRules() {
		nonUnitRules = new ArrayList<HashSet<Integer>>();
		for (int i = 0; i < rules.size(); i++) {
			nonUnitRules.add(new HashSet<Integer>());
		}
		for (int ntID = 0; ntID < rules.size(); ntID++) {
			for (int bodyID : rules.get(ntID)) {
				ArrayList<Integer> body = bodies.get(bodyID);
				if (!(body.size() == 1 && isNonterminal(body.get(0)))) {
					nonUnitRules.get(ntID).add(bodyID);
				}
			}
		}
	}
	...
	/**
	 * Recursively traverse the CFG graph, keeping a list of all the currently
	 * chained nonterminals. If yet another chain rule is found, insert the 
	 * rules of the newly discovered nonterminal into all nonterminals of the 
	 * list. If a non-chained rule is discovered, create an empty list and
	 * keep recursively traversing the graph.
	 */
	private void findUnitRules(int ntID) {
		hasVisitedSymbol[ntID] = true;
		for (int bodyID : rules.get(ntID)) {
			ArrayList<Integer> body = bodies.get(bodyID);
			if (body.size() == 1 && isNonterminal(body.get(0))) {
				if (!hasVisitedSymbol[body.get(0)]) {
					findUnitRules(body.get(0));
				}
				nonUnitRules.get(ntID).addAll(nonUnitRules.get(body.get(0)));
			} else {
				for (int symbolID : bodies.get(bodyID)) {
					if (isNonterminal(symbolID) && !hasVisitedSymbol[symbolID]) {
						findUnitRules(symbolID);
					}
				}
			}
		}
